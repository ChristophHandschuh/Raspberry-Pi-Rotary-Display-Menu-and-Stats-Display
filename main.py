...
def display():
        ...
        if menu == 0:
                ...
                if pm2status != False:
                        processes = str(subprocess.check_output("pm2 status", shell = True), "utf-8").count("online")
                with canvas(device) as draw:
                        draw.text((5, 0), "IP: " + str(subprocess.check_output("hostname -I | cut -d\' \' -f1", shell = True), "utf-8"), fill="white")
                        draw.text((5, 10), str(subprocess.check_output("top -bn1 | grep load | awk '{printf \"CPU: %.2f\", $(NF-2)}'", shell = True), "utf-8"), fill="white")
                        draw.text((5, 20), str(subprocess.check_output("free -m | awk 'NR==2{printf \"Mem: %s/%sMB %.2f%%\", $3,$2,$3*100/$2 }'", shell = True), "utf-8"), fill="white")
                        draw.text((5, 30), str(subprocess.check_output("df -h | awk '$NF==\"/\"{printf \"Disk: %d/%dGB %s\", $3,$2,$5}'", shell = True), "utf-8"), fill="white")
                        draw.text((5, 40), "Temp: " + str(subprocess.check_output("vcgencmd measure_temp |cut -f 2 -d '='", shell = True), "utf-8"), fill="white")
                        if pm2status != False:
